...
| 键位 | 数量 | 功能 | 说明 |
|------|------|------|------|
| K1-K5 | 5 | 首个 WS 的 Agent 状态键 | 显示当前页**第一个 Workspace** 的 Agent，最多 5 个。该 WS 超过 5 个 Agent 时拆多页 |
| K6-K10 | 5 | 次个 WS 的 Agent 状态键 | 显示当前页**第二个 Workspace** 的 Agent，最多 5 个。若当前页只有一个 WS（跨页），本行留空或隐藏 |
| K11 | 1 | 上一页 | 翻到前一页，显示该页首个 WS 名称缩写 |
| K12 | 1 | 当前页 WS 信息 | 显示当前页包含的 WS 名称。单 WS 页显示 `"LCL:WS-A (1/2)"`，双 WS 页显示 `"LCL:main · DEV:beta"`。机器名用配置中的 `abbr` 缩写 |
| K13 | 1 | 下一页 | 翻到后一页，显示该页首个 WS 名称缩写 |
...
┌──────────────┐
│              │
│      π       │  ← Agent 类型图标（SVG, 居中, 占~60% 面积）
│              │
│   review     │  ← 用户自定义别名（Herdr agent name, 小字居中）
│              │
│      ⏳      │  ← 状态图标（右下角或底部居中, 16-20px）
└──────────────┘
```
...
1. 背景色（由状态决定，半透明）
2. Agent 图标（SVG，白色/灰色）
3. 自定义别名文本（白色，小字号）
4. 状态图标（底部居中）
5. 聚焦态边框（2px 白色发光边框，仅 focused=true 时）

**状态→颜色/图标映射：**

| AgentStatus | 背景色 | 状态图标 | 状态英文标签 |
|-------------|--------|---------|------------|
| Done | `#27AE60` (绿) | ✅ | DONE |
| Idle | `#7F8C8D` (灰) | ⏸ | IDLE |
| Working | `#F39C12` (琥珀) | ⏳ | WORKING |
| Blocked | `#E74C3C` (红) | ❌ | BLOCKED |
| Unknown | `#95A5A6` (灰) | ❓ | ? |

**聚焦态：** 2px 白色发光边框 + 背景亮度 +10%
...
键位分布
  ├─ K1-K5:  行1 WS 的 Agents（≤5 个）
  ├─ K6-K10: 行2 WS 的 Agents（≤5 个）
  ├─ K11: 上一页
  ├─ K12: 当前页 WS 信息（`LCL:main · DEV:beta`，用配置中的 abbr）
  ├─ K13: 下一页
  └─ K14: 全局统计栏 (长条, 始终固定)

分页逻辑
  ├─ 统一 WS 列表（多连接按配置顺序合并）
  ├─ 每个 WS 拆成 ≤5 Agent 的 chunks
  ├─ 每页 = 2 个 chunks（行1 + 行2）
  ├─ 双 WS 页：两个 WS 各 ≤5 Agent 时，行1=WS-A，行2=WS-B
  ├─ 单 WS 跨页：一个 WS 有 6-10 个 Agent 时，独占一页或多页
  ├─ 空行处理：页面第二个 chunk 为空时，行2 留空
  └─ 示例：
      ├─ LCL:WS-A(3) + DEV:WS-B(4) → 行1=LCL:WS-A, 行2=DEV:WS-B
      ├─ LCL:WS-A(7) → 第1页 行1=LCL:WS-A[0..4] 行2=LCL:WS-A[5..6]
      └─ LCL:WS-A(10) → 第1页 行1=LCL:WS-A[0..4] 行2=LCL:WS-A[5..9]

导航方式
  └─ 翻页导航 (K11/K13 切换 page)
